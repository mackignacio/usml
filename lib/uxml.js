...
(() => {
  ...
  function stringInterpolation(el, json = {}) {
    ...
    console.log("stringInterpolation", el.textContent);
    if (!textContent)
      return;
  ...
  function loadComponent(id, data) {
    const component = components.get(id);
    if (!component)
      return;
    let props = data ?? getUSMLAction(component, constants_default.USML_PROPS);
    if (!props || component.children.length === 0)
      return;
    props = typeof props === "string" ? stringToJSON(props) : props;
    loadPropsToChildren(component.children, props);
  }
  function loadPropsToChildren(children, data) {
    data = typeof data === "string" ? stringToJSON(data) : data;
    for (const child of children) {
      const updateText = (props) => (
        // Replace interpolated string with json values
        stringInterpolation(child, props)
      );
      updateText({ props: data });
    }
  }
  ...
  function http_default(el, [_cmd, _target], payload) {
    if (typeof _target !== "string")
      return;
    let type = getUSMLAction(el, constants_default.USML_SWAP);
    return matcher(
      _target,
      "http",
      () => http[_cmd](_target, type && !type?.includes("component") ? "text" : "json", payload).then(httpResponseHandler(el)).catch(httpResponseError)
    );
  }
