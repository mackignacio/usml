...
(() => {
  ...
  function mapStringInterpolation(text, json = {}) {
  ...
  // src/lib/signal.ts
  function createSignal(el) {
    const state = { el, subscribers: /* @__PURE__ */ new Set(), props: {} };
    const handler = {
      get: (target, prop) => {
        return target[prop];
      },
  ...
  function updateTextEffect(el, text) {
  ...
  function loadComponent(id, data) {
    ...
    if (!component)
      return;
    let props = data ?? getUSMLAction(component.el, constants_default.USML_PROPS);
    props = typeof props === "string" ? stringToJSON(props) : props;
    if (!props || typeof props === "string" || component.el.children.length === 0)
      return;
